...
const METHODS = [
  ...
  { id:"contacts", label:"Contactos", icon: CONFIG.icons.checks, mode:"contacts" }
];
...
function selectMethod(idSel){
  ...
  // ✅ Contactos: abre modal y no altera el flujo existente
  if(m.mode === "contacts"){
    openContacts();
    return;
  }
  ...
  // ✅ Prefill si vienes de Contactos
  if(state.contact){
...
function renderContacts(){
  ...
  list.forEach(c=>{
    ...
    tr.querySelector("button").onclick = ()=>{
      ...
      cName.value = c.name || "";
      cPhone.value = prettyPhone(c.phone || "");
...
// ✅ “Usar”: guarda y vuelve al grid de métodos
